...
import { User, Sale, Goal, StoreProgress, IndividualProgress } from '../types';
...
export const getCoachingPlan = async (prompt: string, users: User[], individualProgress: IndividualProgress[], goals: Goal | undefined, storeProgress: StoreProgress[]): Promise<string> => {
  if (!prompt.trim()) {
...
export const getDetailedReportAnalysis = async (users: User[], goal: Goal | undefined, storeProgress: StoreProgress[], individualProgress: IndividualProgress[]): Promise<string> => {
    const model = 'gemini-2.5-flash';
