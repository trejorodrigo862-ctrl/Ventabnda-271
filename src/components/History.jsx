...
const MetricRow = ({ label, achieved, goal, isCurrency }) => {
    const formatValue = (val) => isCurrency ? `$${val.toLocaleString('es-AR')}` : val.toLocaleString('es-AR');
    return (
        <div>
...
const DetailView = ({ month, onBack }) => {
                ...
                .reduce((userAcc, p) => {
                    (Object.keys(p)).forEach(key => {
                         if (key !== 'id' && key !== 'date' && key !== 'userId' && typeof p[key] === 'number') {
                            userAcc[key] = (userAcc[key] || 0) + (p[key]);
                        }
                    });
                    return userAcc;
                }, {});
    ...
    const teamGoal = goalForMonth.teamGoal;
                    ...
                    {users.filter(u => u.role !== 'Encargado').map(user => {
                        ...
                        return (
                            <div key={user.id} className="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-md border border-gray-200 dark:border-gray-800">
                                ...
                                {user.role === 'Vendedor' && (
                                    <>
                                        <MetricRow label="Venta ($)" achieved={userAchieved.pesos || 0} goal={(userGoal).metaPesos || 0} isCurrency />
                                        <MetricRow label="Unidades" achieved={userAchieved.unidades || 0} goal={(userGoal).metaUnidades || 0} />
                                        <MetricRow label="Calzado (U)" achieved={userAchieved.calzado || 0} goal={(userGoal).metaCalzado || 0} />
                                        <MetricRow label="Indumentaria (U)" achieved={userAchieved.indumentaria || 0} goal={(userGoal).metaIndumentaria || 0} />
                                    </>
                                )}
                                 {user.role === 'Cajero' && (
                                    <>
                                        <MetricRow label="MC Crédito ($)" achieved={userAchieved.pesosMcCred || 0} goal={(userGoal).metaPesosMcCred || 0} isCurrency />
                                        <MetricRow label="MC Crédito (U)" achieved={userAchieved.unidadesMcCred || 0} goal={(userGoal).metaUnidadesMcCred || 0} />
                                        <MetricRow label="Medias (U)" achieved={userAchieved.medias || 0} goal={(userGoal).metaMedias || 0} />
                                    </>
                                )}
                                </div>
                            </div>
