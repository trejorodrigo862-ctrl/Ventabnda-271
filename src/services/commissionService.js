...
const managerPerformanceWeights = [
...
const calculateCommissionValue = (score, tiers) => {
...
export const calculateCommissionData = ({ user, currentGoal, storeProgress, individualProgressForUser, simulationInput = {} }) => {
    ...
    if (user.role === 'Encargado') {
        ...
        return {
            // Using 'as const' to help TypeScript with discriminated union type inference.
            role: 'Encargado',
    ...
    Object.keys(simulationInput).forEach(key => {
        aggregatedIndividualProgress[key] = (aggregatedIndividualProgress[key] || 0) + (simulationInput[key] || 0);
    });
    ...
    if (user.role === 'Vendedor') {
        const userGoals = currentGoal.userGoals[user.id];
