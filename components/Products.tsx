...
import { User, Goal, UserRole, TeamGoalSet, VendedorGoalSet, CajeroGoalSet, Message } from '../types';
...
interface TeamFormProps {
  onSubmit: (user: Omit<User, 'id' | 'assignedHours'>) => void;
  onClose: () => void;
...
const Team: React.FC<TeamProps> = ({ viewMode }) => {
  ...
  const handleFormSubmit = (userData: Omit<User, 'id' | 'assignedHours'>) => {
    if (userToEdit) {
  ...
  const handleTeamGoalChange = (goalKey: keyof TeamGoalSet, value: string | number) => {
  ...
  const handleSaveGoals = () => {
    ...
    const assignedHoursSnapshot: { [userId: string]: number } = {};
    users.forEach(user => {
  ...
  const handleHourChange = (userId: string, hours: string) => {
